...
script="dockConfig"
logFolder="/Library/Logs/Microsoft/IntuneScripts/$script"
log=${logFolder}/$script.log
mkdir -p "$logFolder"
exec &> >(tee -a "$log")
...
if [[ "$useDockUtil" == true ]]; then
    ...
    echo "$(date) | Current user home $userHome"
    echo "$(date) | Current user dock plist $plist"
    install_dockutil_if_missing
    configure_dock_with_dockutil
else
